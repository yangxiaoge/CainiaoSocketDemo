 ...
 * 三个线程的情况
 */
public class SocketBarcodeUtil_bak {
    ...
    // 单例模式
    private static SocketBarcodeUtil_bak instance;
    // 全局的线程池
     ...
     * @return Application
     */
    public static SocketBarcodeUtil_bak getInstance() {
        if (instance == null)
            instance = new SocketBarcodeUtil_bak();
...
//        String ips = FileIOUtils.readFile2String(filePath);
    ...
    private boolean dealWithData3(InputStream is, XData xData) {
        try {
            ...
            saveImage3(calendar, xData, imgBitmap);
            return true;
    ...
    private boolean dealWithData2(InputStream is, XData xData) {
        try {
            ...
            final Bitmap imgBitmap = BitmapFactory.decodeByteArray(bos.toByteArray(), 0, bos.toByteArray().length);
            Calendar calendar = Calendar.getInstance();  //获取当前时间，作为图标的名字
            saveImage2(calendar, xData, imgBitmap);
            return true;
     ...
     * @param bmp   bitmap
     */
    private void saveImage(Calendar calendar, XData xData, final Bitmap bmp) {
     ...
     * @param bmp   bitmap
     */
    private void saveImage2(Calendar calendar, XData xData, final Bitmap bmp) {
        // TODO: 2018/6/26 测试时先注释掉
        if (data.contains(xData.barcode)) return;//已存在
        System.out.println("saveImage保存图片1");
        String year = calendar.get(Calendar.YEAR) + "";
        ...
        try {
            ...
            xData.imgPath = file.getAbsolutePath();
            savaBarcodeInfo2(calendar, xData.barcode, xData.deviceNum, xData.imgname, xData.imgPath);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     * @param bmp   bitmap
     */
    private void saveImage3(Calendar calendar, XData xData, final Bitmap bmp) {
        // TODO: 2018/6/26 测试时先注释掉
        if (data.contains(xData.barcode)) return;//已存在
        System.out.println("saveImage保存图片1");
        String year = calendar.get(Calendar.YEAR) + "";
        ...
        try {
            ...
            xData.imgPath = file.getAbsolutePath();
            savaBarcodeInfo3(calendar, xData.barcode, xData.deviceNum, xData.imgname, xData.imgPath);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//        String fileName = time + ".txt";
     ...
     * @param imgPath   图片绝对（全）路径
     */
    private void savaBarcodeInfo2(Calendar calendar, String barcode, String deviceNum,
                                  String imgName, String imgPath) {
        // TODO: 2018/6/26 测试时先注释掉
        if (data.contains(barcode)) return;//已存在
        String year = calendar.get(Calendar.YEAR) + "";
...
//        String fileName = time + ".txt";
     ...
     * @param imgPath   图片绝对（全）路径
     */
    private void savaBarcodeInfo3(Calendar calendar, String barcode, String deviceNum,
                                  String imgName, String imgPath) {
        // TODO: 2018/6/26 测试时先注释掉
        if (data.contains(barcode)) return;//已存在
        String year = calendar.get(Calendar.YEAR) + "";
